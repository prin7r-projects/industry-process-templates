...
This Standard Operating Procedure defines the step-by-step process for pre-season equipment inventory audit. Following this SOP ensures consistency, quality, and compliance across all engagements in the hvac vertical.
...
Begin the core execution of pre-season equipment inventory audit by establishing the working environment: (1) Create a new working directory with today's date and procedure name, (2) Copy all input files into the working directory (never work on originals), (3) Initialize the tracking spreadsheet with baseline metrics, (4) Set up any required virtual environments, database connections, or API sessions, (5) Take a system state snapshot using the provided diagnostic tool. Record the start time in the procedure log (LOG-001).
...
After processing completes, verify the results: (1) Run the automated verification script (verify-pre-season-equipment-inventory-audit.sh) which checks record counts, hash integrity, and business rule compliance, (2) Manually review a random sample of 5% of records (minimum 25 records) for accuracy, (3) Compare key metrics against the previous period's results — investigate any variance >10%, (4) Generate the verification report (RPT-VRF-001) and attach it to the procedure log.
